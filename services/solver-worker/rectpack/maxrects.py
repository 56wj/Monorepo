...
def update_list_based_on_string(lst, input_str):
    # 分离输入字符串的前后部分，*前的部分和*后的部分
    str_before_star, str_after_star = input_str.rsplit('*', 1)

    # 遍历列表
    for i in range(len(lst)):
        # 分离列表元素的前后部分，*前的部分和*后的部分
        list_before_star, list_after_star = lst[i].rsplit('*', 1)

        # 如果*前的部分相同，则进行更新
        if list_before_star == str_before_star:
            # 将*后的部分转为数字，并进行加法操作
            new_value = int(list_after_star) + int(str_after_star)
            # 更新列表中的该元素，*后的部分替换为加法结果
            lst[i] = f"{list_before_star}*{new_value}"
            return lst
        lst.append(input_str)
    return lst


def update_count_based_on_string(num, input_str):
    str_before_star, str_after_star = input_str.rsplit('*', 1)
    num += int(str_after_star)
    return num
...
class MaxRects(PackingAlgorithm):
    ...
    def _rect_fitness(self, max_rect, width, height):
        ...
        if width <= max_rect.width and height <= max_rect.height:
            return 0
            # return (max_rect.width - width) * (max_rect.height - height)
            # return max_rect.width - width
        else:
            return None

    def _select_position(self, w, h, rule):
        """
        ...
        # Normal rectangle
        if rule:
            fitn = ((self._rect_fitness(m, w, h), w, h, m, False) for m in self._max_rects
                    if self._rect_fitness(m, w, h) is not None)
        else:
            fitn = ((self._rect_fitness(m, w, h), w, h, m, False) for m in self._max_rects
                    if self._rect_fitness(m, w, h) is not None and m.x != 0)

        # Rotated rectangle
        fitr = ((self._rect_fitness(m, h, w), h, w, m, True) for m in self._max_rects
        ...
        try:
            _, w, h, m, b = min(fit, key=first_item)
        except ValueError:
            return None, None

        return Rectangle(m.x, m.y, w, h, rotation=b), m
    ...
    def fitness(self, width, height, rule=True):
        """
        ...
        rect, max_rect = self._select_position(width, height, rule)
        if rect is None:
            return None
    ...
    def add_rect(self, width, height, rid=None, rule=True):
        """
        ...
        rect, _ = self._select_position(width, height, rule)
        if not rect:
            update_list_based_on_string(self.unpack_items, rid[1])
            update_count_based_on_string(self.unpack_items_count, rid[1])
            return None
...
class MaxRectsBbef(MaxRects):
    """The best bottom edge fit the multiple fit requirement"""
    def _rect_fitness(self, max_rect, width, height):
        if width > max_rect.width or height > max_rect.height:
            return None

        result1 = (max_rect.x + max_rect.y) * 100 - width / max_rect.width
        result2 = (max_rect.x + max_rect.y) * 100 - (width * 2 / max_rect.width if width * 2 <= max_rect.width else 0)
        result3 = (max_rect.x + max_rect.y) * 100 - (width * 3 / max_rect.width if width * 3 <= max_rect.width else 0)
        return min(result1, result2, result3)


class MaxRectsBiof(MaxRects):
    """The best bottom edge fit the multiple fit requirement"""
    def _rect_fitness(self, max_rect, width, height):
        if width > max_rect.width or height > max_rect.height:
            return None

        return max_rect.y*100 + max_rect.x
