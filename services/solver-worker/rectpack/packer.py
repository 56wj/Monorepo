...
class PackerBBFMixin(object):
    ...
    def add_rect(self, width, height, rid=None, rule=True):
        ...
        try:
            ...
            best_bin.add_rect(width, height, rid, rule)
            return True
        except ValueError:
            pass    
        ...
        while True:
            ...
            # _new_open_bin may return a bin that's too small,
            # so we have to double-check
            if new_bin.add_rect(width, height, rid, rule):
                return True
...
class PackerOnline(object):
    ...
    def add_bin(self, width, length, count=1, height=0, max_weight=0, name='', **kwargs):
        # accept the same parameters as PackingAlgorithm objects
        kwargs['rot'] = self._rotation
        bin_factory = BinFactory(width, length, count, self._pack_algo, **kwargs)
    ...
    def rect_list(self):
        ...
        for abin in self:
            for rect in abin:
                rectangles.append((bin_count, rect.x, rect.y, rect.width, rect.height, rect.rid, rect.rotation))
            bin_count += 1
...
class Packer(PackerOnline):
    ...
    def __init__(self, pack_algo=MaxRectsBssf, sort_algo=SORT_NONE, 
        ...
        self._sorted_rect = []
        self._unpack_item = []
        self._unpack_item_count = 0

    def add_bin(self, width, length, count=1, height=0, max_weight=0, name='', **kwargs):
        self._avail_bins.append((width, length, count, height, max_weight, name, kwargs))
    ...
    def pack(self, rule_vertical_wall):
        ...
        for b in self._avail_bins:
            width, length, count, height, max_weight, name, extra_kwargs = b
            super(Packer, self).add_bin(width, length, count, height, max_weight, name, **extra_kwargs)
        ...
        for r in self._sorted_rect:
            if not super(Packer, self).add_rect(*r, rule_vertical_wall):
                self._unpack_item = update_list_based_on_string(self._unpack_item, r[2][1])
                self._unpack_item_count = update_count_based_on_string(self._unpack_item_count, r[2][1])
...
def newPacker(mode=PackingMode.Offline, bin_algo=PackingBin.BBF, pack_algo=MaxRectsBssf, sort_algo=SORT_AREA,
              rotation=True):
    """
