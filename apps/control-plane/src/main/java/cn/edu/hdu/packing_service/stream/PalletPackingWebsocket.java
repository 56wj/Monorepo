...
@ServerEndpoint(value = "/palletpackingWebsocket", encoders = {ResultEncoder.class})
@Component
public class PalletPackingWebsocket {

    private static final long MAX_IDLE_TIMEOUT_MS = 90000L;

    /** sid -> WebSocket session */
    public static final Map<String, Session> clients = new ConcurrentHashMap<>();

    /** userId -> sids；同一账号可以同时打开多个页面。 */
    ...
    public static void sendMessageByUserId(String userId, Result message) {
        ...
        for (String sid : clientSet) {
            ...
            try {
                // BasicRemote 不允许同一 Session 并发发送，按 Session 串行化。
                synchronized (session) {
    ...
    public void onMessage(String message, Session session) {
        System.out.println(DateUtil.getNowTime() + "收到来自窗口" + this.userId + "的信息:" + message);
    }
