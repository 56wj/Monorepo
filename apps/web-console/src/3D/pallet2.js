...
function onMouseClick(event) {
	...
	if(intersections.length > 0) {
        ...
        css2DObject.position.x = intersected.parent.position.x;
        css2DObject.position.y = intersected.parent.position.y + intersected.geometry.parameters.height / 2 + 45;
        css2DObject.position.z = intersected.parent.position.z;
        console.log(css2DObject);
        modifyDocument("lableItemId", "red", defaults.nameList[intersected.itemId-1]);
	}else{
...
function getInfoDetail(data) {
    ...
    for (let i = 0; i < defaults.detailNum; i++) {
        ...
        defaults.nameList.push({show_name1:detail.show_name1, show_name2:detail.show_name2});
    }
    console.log(defaults.nameList);
};
// 初始化车箱(车箱)
...
function addCSS2DLabelToScene() {
    ...
    // if (!element) {
    //     console.error("Element with id 'WebGL-output' not found!");
    // }
    ...
    // webglOutput.style.width = "100px";
    // webglOutput.style.height = "100px";

    // 创建子元素 #lableItem
    const labelItem = document.createElement('div');
    labelItem.className = "css_style";
    labelItem.id = "lableItem";

    // 创建子元素 #lableItemId 并设置初始文本内容
    const labelItemId = document.createElement('span');
    labelItemId.className = "css3dTitleWarning";
    labelItemId.id = "lableItemId";
    labelItemId.innerText = "00000000000";

    // 将子元素添加到父元素中
    labelItem.appendChild(labelItemId);
    webglOutput.appendChild(labelItem);
    
    // 将元素添加到 body
    document.body.appendChild(webglOutput);

    //把生成的CSSDOM对象处理成three的节点对象
    css2DObject = new CSS2DObject(webglOutput);
    console.log("new css2dobject finsih");
    //设置CSS2DObject对象
    css2DObject.position.x = 0;
    css2DObject.position.y = 0;
    css2DObject.position.z = 0;
    console.log("set css2dobject finsih");
    //在第二个场景中添加这个对象
    scene.add(css2DObject);
    console.log("add css2dobject finsih");
    // 默认不显示
    css2DObject.visible = false;
}
function modifyDocument(id, color, value) {
    ...
    dom.textContent = value.show_name1 + '\n' + value.show_name2;
}
...
function init(data) {
    ...
    document.getElementById('3D_chart').appendChild( labelRenderer.domElement );
...
function clearScene(scene) {
    ...
    while(scene.children.length > 0){ 
        ...
        scene.remove(obj);
            ...
            // 如果是 CSS2DObject 类型的对象，进行 DOM 相关的清理
