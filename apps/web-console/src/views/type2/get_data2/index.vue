<template>
  <div class="dashboard-container">
    <el-form ref="form" :model="form" :rules="rules" label-width="100px">
      <el-tabs v-model="activeName" @tab-click="" style="box-shadow: 6px 6px 6px rgba(0,0,0,0.1)">
        <el-tab-pane label="规则设置" name="first">
          <el-row style="margin-top: 20px">
            <el-col :span=12>
              <el-form-item label="叠膜规则：" label-width="220px">
                <el-select v-model="form.overlap_way">
                  <el-option v-for="item in options_overlap_way" :key="item.value" :label="item.label" :value="item.value">
                  </el-option>
                </el-select>
              </el-form-item>
            </el-col>
          ...
          <el-row>
            <el-col :span=12>
              <el-form-item label="挡板默认厚度(cm)：" label-width="220px">
                <!-- <el-input placeholder="请输入" v-model.number="form.baffle_thickness" style="width: 250px" clearable>
                </el-input> -->
                <el-input placeholder="请输入" v-model="form.baffle_thickness" @input="exitCheck_baffle_thickness(form.baffle_thickness)" @blur="exitSave_baffle_thickness(form.baffle_thickness)" style="width: 250px" clearable>
                </el-input>
              </el-form-item>
            </el-col>
            <el-col :span=12>
              <el-form-item label="托盘：" label-width="220px">
                <el-select v-model="form.tray">
                  <el-option v-for="item in options_tray" :key="item.value" :label="item.label" :value="item.value">
                  </el-option>
                </el-select>
              </el-form-item>
            </el-col>
          ...
          <el-row>
            <el-col :span=12>
              <el-form-item label="膜卷宽度阈值(mm)：" label-width="220px" prop="overlap_width">
                <el-input placeholder="请输入" v-model.number="form.overlap_width" style="width: 250px" clearable>
                </el-input>
              </el-form-item>
            </el-col>
            <el-col :span=12>
              <el-form-item label="挡板长度增加值(cm)：" label-width="220px" prop="baffle_extra">
                <el-input placeholder="请输入" v-model.number="form.baffle_extra" style="width: 250px" clearable>
                </el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span=12>
              <el-form-item label="可否允许垂直侧壁：" label-width="220px">
                <el-radio-group v-model="form.rule_vertical_wall">
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="辅材到货日期：" label-width="220px">
                <el-date-picker type="date" placeholder="选择日期" v-model="form.recDate"></el-date-picker>
              </el-form-item>
            </el-col>
            <el-col :span=12>
    ...
    <el-dialog title="规格确认表" :visible.sync="dialogTableVisible" :before-close="handleClose">
      <el-table :data="dialog" :key="forceRefresh" height=530px border>
        ...
        <el-table-column property="often_number" label="规格表个数" width="200">
          <template slot-scope="scope">
            <div class="txt">{{ tray1111(scope.row) }}</div>
          </template>
        </el-table-column>
...
<script>
import { post_al_first } from '@/api/type2'
...
export default {
  ...
  data() {
    return {
      activeName: 'first',
      form: {
        overlap_way: '1托3卷/6卷/托',
        baffle_thickness: 2.5, 
        tray: '木托',
        overlap_width: 700,
        baffle_extra: 3,
        default_density: 0.905, // 默认密度
        rule_vertical_wall: true,
        orderId: '',
        box_list: [{
          box_id: '',
          num: 1
  ...
  methods: {
      ...
      try {
        ...
        if ("source" in this.$route.query && this.$route.query.source.config != null){
          ...
          if("push_id" in this.$route.query && this.$route.query.push_id != null){
            ...
            const res3 = await get_task(data)
            this.$data.form.orderId = res3.data.orderId
            this.$data.tableData = res3.data.orderData.tableData
    ...
    uploadData(param) {
      ...
      this.$refs['form'].validate((valid,item) => {
        if(!valid){
          alert("规则设置有误，请检查！");
        }else if(palletDataCheck(this.$data.tableData)){
          console.log('submit!');
          this.to_upload();
        }
    ...
    //  console.log(2)
      ...
      fileReader.onload = (files) => {
        try {
          ...
          console.log(this.$data.xlsxData)
    ...
    exitCheck_baffle_thickness(num) {
      this.$data.form.baffle_thickness = check(num)
    },
    exitSave_baffle_thickness(num) {
      this.$data.form.baffle_thickness = save(num)
    },
