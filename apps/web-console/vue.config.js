...
module.exports = {
  ...
  devServer: {
    // host: 'localhost',
    host: '0.0.0.0',//  docker上运行要设置为0.0.0.0
    port: port,
    open: true,
    overlay: {
      warnings: false,
    ...
    proxy: {
      [process.env.VUE_APP_BASE_API]: {
        target: process.env.VUE_APP_PROXY_TARGET || 'http://localhost:8101', // 后端请求地址
      ...
      '/palletpackingWebsocket': {
        target: process.env.VUE_APP_WS_PROXY_TARGET || process.env.VUE_APP_PROXY_TARGET || 'http://localhost:8101',
        changeOrigin: true,
        ws: true
      },
